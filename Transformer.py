...
@register_model
def deit_small_patch16_224(embed_dim=384,depth=8,num_heads=8,pretrained=False,**kwargs):
    model = DeiT(
        patch_size=16, embed_dim=embed_dim, depth=depth, num_heads=num_heads, mlp_ratio=4, qkv_bias=True,
    ...
    pe = F.interpolate(pe, size=(7, 7), mode='bilinear', align_corners=True)
    pe = pe.flatten(2)
    pe = pe.transpose(-1, -2)
